...
flags.DEFINE_string('datadir', 'dataset/data-002.txt', 'set the data root directory')
...
if keep == True:
	...
	if mode == 'test':
		...
		print('The recommended price is between ' + str(float(predictions[0][0]) * 1000) + ' and ' + str(float(predictions[0][0]) * 1000 + 999))


	if mode == 'train':
		# load dataset
		with open(datadir) as infile:
			dataset = np.zeros((22384 , 128, 128, 3)) # 128 * 128 * 3
			a = 0
			for line in infile:
				graph = np.fromstring(line, dtype = int, sep = ",")
				for i in range(0, int((graph.size / 3))):
